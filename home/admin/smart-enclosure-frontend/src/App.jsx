...
export default function App() {
  ...
  return <div className="app"><header className="topbar"><a href="#overview" className="brand"><span className="brand-mark" aria-hidden="true">b</span>smart enclosure</a><nav aria-label="Main navigation"><a href="#overview" aria-current={page === 'overview' ? 'page' : undefined}>Overview</a><a href="#settings" aria-current={page === 'settings' ? 'page' : undefined}>Settings</a><a href="/debug.html">Debug ↗</a></nav><div className="segmented units" aria-label="Temperature unit">{['F', 'C'].map(u => <button key={u} aria-pressed={unit === u} onClick={() => chooseUnit(u)}>°{u}</button>)}</div></header>
